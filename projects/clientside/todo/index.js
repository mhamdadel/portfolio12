...
JSON.parse(localStorage["todo-items"]).forEach((el) => {
    const todoList = [...document.querySelectorAll("#todo-list")][el["parent"]];
    const finishedList = [...document.querySelectorAll("#finished-list")][
        el["parent"]
    ];
    const newChiled = `
<p
...
function drop(event) {
    ...
    if (
        ...
        document.getElementById(event.target.id).classList.contains("todo-item")
    ) {
        ...
        [...items].forEach(function (el) {
            ...
            if (
                ...
                let nodeOne = todoItemsInStorage[getIndexOfTodoItemById(data)];
                let nodeTwo = todoItemsInStorage[getIndexOfTodoItemById(el.id)];
                todoItemsInStorage[getIndexOfTodoItemById(data)] = nodeTwo;
                todoItemsInStorage[getIndexOfTodoItemById(el.id)] = nodeOne;
                localStorage.setItem(
                    "todo-items",
...
function removeGrandParent(t) {
    ...
    let todoItemsInStorage = JSON.parse(localStorage["todo-items"]);
    todoItemsInStorage.splice(getIndexOfTodoItemById(element.id), 1);
    localStorage.setItem("todo-items", JSON.stringify(todoItemsInStorage));
...
function swap(t) {
    ...
    let myElmentsInStore = JSON.parse(localStorage.getItem("todo-items"));
    let myElmentInStore = myElmentsInStore[getIndexOfTodoItemById(element.id)];
    if (myElmentInStore["finished"] == 0) {
        myElmentInStore["finished"] = 1;
    } else if (myElmentInStore["finished"] == 1) {
        myElmentInStore["finished"] = 0;
    }
    localStorage.setItem("todo-items", JSON.stringify(myElmentsInStore));
}
...
function swapDrag(element) {
    ...
    if (checkBox.checked && parent.id.startsWith("todo-list")) {
        finishedList.appendChild(element);
    } else if (!checkBox.checked && parent.id.startsWith("finished-list")) {
        todoList.appendChild(element);
    }
